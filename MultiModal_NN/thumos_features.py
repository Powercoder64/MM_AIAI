...
class ThumosFeature(data.Dataset):
    def __init__(self, data_path, mode, modal, feature_fps, num_segments, len_feature, sampling, filename, seed=-1, supervision='no_weak'):
        if seed >= 0:
            utils.set_seed(seed)
        ...
        if self.modal == 'all':
            ...
            for _modal in ['rgb', 'flow']:
                feature_file = filename[0:-4] + '_{}.npy'.format(_modal)
                #print(feature_file)
                self.feature_path.append(os.path.join(data_path + '/features/', feature_file))
        #else:
            #self.feature_path = os.path.join(data_path, filename)
        ...
        #split_file.close()
        self.vid_list = os.listdir(data_path + '/video/')
        #filter vidlist to remove any files that are not .mp4
        self.vid_list = [vid for vid in self.vid_list if vid.endswith('.mp4')]
    ...
    def __getitem__(self, index):
        ...
        # Ensure that rgb_feature has the expected dimensions before accessing its shape
        if hasattr(data, 'shape') and len(data.shape) > 0:
            vid_num_seg = data.shape[0]
        else:
            # Handle the case where data is not as expected
            raise ValueError(f"Data at index {index} does not have the expected shape or is empty.")
    ...
    def get_data(self, index):
        ...
        # try:
        #    file_exists = check_file_with_retries(self.feature_path[0], max_retries=5, delay_seconds=30)
        # except FileNotFoundError as e:
        #    print(e)

        # try:
        #    file_exists = check_file_with_retries(self.feature_path[1], max_retries=5, delay_seconds=30)
        # except FileNotFoundError as e:
        #    print(e)

        if self.modal == 'all':
            #print(self.feature_path[0], vid_name)
            #print(self.feature_path[1], vid_name)
            rgb_feature = np.load(self.feature_path[0]).astype(np.float32)
            flow_feature = np.load(self.feature_path[1]).astype(np.float32)
